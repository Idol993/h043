...
from constants import (
    ...
    ENEMY_PATROL, ENEMY_TRACKER, ENEMY_GUARD, ENEMY_ELITE,
    COLOR_ENEMY_PATROL, COLOR_ENEMY_TRACKER, COLOR_ENEMY_GUARD, COLOR_ENEMY_ELITE,
...
class Renderer:
    ...
    def draw_enemies(self,enemies):
        cols={ENEMY_PATROL:COLOR_ENEMY_PATROL,ENEMY_TRACKER:COLOR_ENEMY_TRACKER,ENEMY_GUARD:COLOR_ENEMY_GUARD,ENEMY_ELITE:COLOR_ENEMY_ELITE}
        hl={ENEMY_PATROL:(255,100,100),ENEMY_TRACKER:(220,120,255),ENEMY_GUARD:(255,200,100),ENEMY_ELITE:(255,180,80)}
        for e in enemies:
            er=e.rect.move(-self.camera.x,-self.camera.y)
            if not self._iv(er):continue
            if e.etype==ENEMY_ELITE:pygame.draw.rect(self.screen,(100,40,20),(er.x-2,er.y-2,er.w+4,er.h+4))
            ...
            elif e.etype==ENEMY_ELITE:
                pygame.draw.rect(self.screen,(255,200,0),(er.centerx-3,er.y+2,6,6))
                pygame.draw.rect(self.screen,(255,200,0),(er.x+2,er.bottom-6,4,4));pygame.draw.rect(self.screen,(255,200,0),(er.right-6,er.bottom-6,4,4))
    ...
    def draw_player(self,player):
        ...
        pygame.draw.rect(self.screen,COLOR_PLAYER,pr);pygame.draw.rect(self.screen,COLOR_PLAYER_TOP,(pr.x+2,pr.y+2,pr.w-4,pr.h-8));self._eye(pr,pr.y+6)
    def draw_hud(self,player,level):
        ...
        if self.font:
            ...
            lt=self.font.render(f"第 {level} 层",True,COLOR_TEXT);self.screen.blit(lt,lt.get_rect(topright=(self.camera.view_w-10,10)))
        y=by+bh+40
        for b in player.buffs:
            bc=BUFF_COLORS.get(b,COLOR_TEXT);pygame.draw.rect(self.screen,bc,(10,y,12,12));pygame.draw.rect(self.screen,(255,255,255),(10,y,12,12),1)
            if self.sfont:nm=BUFF_NAMES.get(b,"?");self.screen.blit(self.sfont.render(nm,True,bc),(26,y-2))
            y+=18
        combos=player.get_combos()
        if combos and self.sfont:
            self.screen.blit(self.sfont.render("[组合]",True,(255,220,80)),(10,y-2));y+=16
            for cb in combos:
                pygame.draw.rect(self.screen,(255,200,50),(10,y,12,12));pygame.draw.rect(self.screen,(255,255,255),(10,y,12,12),1)
                self.screen.blit(self.sfont.render(cb,True,(255,220,80)),(26,y-2));y+=18
            if y>self.camera.view_h-20:return
    def _overlay(self,title,lines,hint=""):
        ov=pygame.Surface((self.camera.view_w,self.camera.view_h),pygame.SRCALPHA);ov.fill((0,0,0,180));self.screen.blit(ov,(0,0))
        if not self.font:return
        cx,cy=self.camera.view_w//2,self.camera.view_h//2
        t=self.font.render(title,True,(255,200,100));self.screen.blit(t,t.get_rect(center=(cx,cy-60-len(lines)*10)))
        for i,l in enumerate(lines):
            c=(255,220,80) if l.startswith("[") else COLOR_TEXT
            t=self.font.render(l,True,c);self.screen.blit(t,t.get_rect(center=(cx,cy-20+i*28)))
        if hint:t=self.font.render(hint,True,(150,150,150));self.screen.blit(t,t.get_rect(center=(cx,cy+20+len(lines)*28)))
    ...
    def draw_pause(self,player,level):
        ...
        ci=player.get_combo_info()
        if ci:ls.append("[能力组合]")
        for k,v in ci.items():ls.append(f"  {k}: {v}")
        self._overlay("暂停",ls,"按 P 继续")
    def draw_level_up(self,options,sel):
        ov=pygame.Surface((self.camera.view_w,self.camera.view_h),pygame.SRCALPHA);ov.fill((0,0,0,180));self.screen.blit(ov,(0,0))
        if not self.font:return
        cx,cy=self.camera.view_w//2,self.camera.view_h//2
        t=self.font.render("选择强化!",True,(255,200,100));self.screen.blit(t,t.get_rect(center=(cx,cy-110)))
        for i,opt in enumerate(options):
            is_sel=(i==sel);c=(255,255,100) if is_sel else COLOR_TEXT;bc=BUFF_COLORS.get(opt,COLOR_TEXT);bx,by2=cx-140,cy-60+i*60
            if is_sel:pygame.draw.rect(self.screen,(60,60,80),(bx-6,by2-4,290,52),border_radius=4)
            pygame.draw.rect(self.screen,bc,(bx,by2+4,24,24));pygame.draw.rect(self.screen,(255,255,255),(bx,by2+4,24,24),1)
            self.screen.blit(self.font.render(BUFF_NAMES.get(opt,"?"),True,c),(bx+32,by2))
            if self.sfont:self.screen.blit(self.sfont.render(BUFF_DESCS.get(opt,""),True,(180,180,180) if is_sel else (120,120,120)),(bx+32,by2+24))
        if self.sfont:ht=self.sfont.render("↑↓选择 Enter确认",True,(150,150,150));self.screen.blit(ht,ht.get_rect(center=(cx,cy+140)))
